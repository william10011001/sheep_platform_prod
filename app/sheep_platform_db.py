...
def _help_icon_html(text: str) -> str:
    ...
    return (
        '<span class="help_wrap" tabindex="0" role="button">'
        '<span class="help_icon" tabindex="0" aria-hidden="true">?</span>'
...
def _render_user_hud(user: Dict[str, Any]) -> None:
    ...
    points_help = (
        "積分的規則：每跑過一個達標組合，且因子池系統在一週實盤結算後的 USDT 利潤，其中一半會換算成你的積分。積分可用於兌換空投獎勵，兌換與發放以活動規則與結算結果為準。"
    )
