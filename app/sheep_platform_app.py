...
from sheep_review import count_review_pipeline_tasks, enrich_task_row, normalize_review_fields
...
def _init_once() -> None:
    ...
    admin_username = str(os.environ.get("SHEEP_BOOTSTRAP_ADMIN_USER", "") or "").strip()
    admin_password = str(os.environ.get("SHEEP_BOOTSTRAP_ADMIN_PASS", "") or "").strip()
    ...
    try:
        if row:
            conn = db._conn()
            try:
                # Keep the bootstrap admin account enabled, but do not rotate passwords implicitly.
                conn.execute(
                    "UPDATE users SET role = 'admin', disabled = 0 WHERE id = ?",
                    (int(row["id"]),),
                )
                conn.commit()
                db.log_sys_event("BOOTSTRAP", None, "已確認管理員帳號存在並啟用", {"username": admin_username})
            finally:
                conn.close()
        else:
...
def _page_dashboard(user: Dict[str, Any]) -> None:
    try:
        ...
        tasks = [enrich_task_row(t) for t in all_tasks if t.get("cycle_id") == int(cycle["id"])]
        ...
        st.markdown(_render_kpi("策略", active_strategies, "個人已上線"), unsafe_allow_html=True)
        st.markdown(_render_kpi("策略", review_pipeline_count, "達標 / 自動管理中"), unsafe_allow_html=True)
        st.markdown(_render_kpi("全網", global_active_strategies, "實盤因子總數"), unsafe_allow_html=True)
...
def _page_tasks(user: Dict[str, Any], job_mgr: JobManager) -> None:
    ...
    def _render_single_task(task_obj: Dict[str, Any], v_status: str, is_active: bool):
        ...
        try:
            ...
            prog = {}
        review_info = normalize_review_fields(prog, v_status)
        ...
        best_any_passed = bool(review_info.get("best_any_passed") or False)
        review_status = str(review_info.get("review_status") or "not_eligible")
        review_reason = str(review_info.get("review_reason") or "").strip()
        ...
        passed_label = review_label_map.get(review_status, review_status or ("條件符合" if best_any_passed else "未達閾值"))
        ...
        review_pill_kind = "ok" if review_status in ("auto_managed", "queued", "running", "passed") else ("bad" if review_status in ("rejected", "error", "not_eligible") else "neutral")
        ...
        st.markdown(f'<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px;">'
                    ...
                    f'<span class="pill pill-{review_pill_kind}">審核: {passed_label}</span>'
                    f'</div>'
        ...
        if last_error:
            st.error(f"核心防護觸發：\n\n{last_error}")
        elif review_status in ("rejected", "error", "not_eligible") and review_reason:
    ...
    with tab2:
        if not failed_cands:
            ...
            for t_obj, c, audit in failed_cands:
                m = c.get("metrics") or {}
                if audit:
                    reason = ", ".join(audit.get("reason_messages", [])) if audit.get("reason_messages") else audit.get("error", "條件未達標")
                else:
                    reason = "未知錯誤"
                rows.append({
                    "候選編號": c["id"],
...
def _render_audit(audit: Dict[str, Any]) -> None:
    ...
    if audit.get("reason_messages"):
        st.write("原因", audit.get("reason_messages"))
    elif audit.get("reasons"):
