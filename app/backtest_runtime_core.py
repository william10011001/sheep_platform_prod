# backtest_runtime_core.py
# ------------------------------------------------------------
# 後端安全的多指標格點搜尋回測 runtime
# - 嚴謹的交易模型：次一根開盤入場、內含 TP/SL/持倉m出場、0.02% 單邊費用、0%滑點
# - 30+ 技術指標本地計算，供格點掃描
# - 可選 Numba JIT 加速（如環境支援），不支援則自動降級
# - 不在模組匯入時初始化 Streamlit，供 CLI / worker / Holy Grail 背景流程共用
# ------------------------------------------------------------
...
_DATAFRAME_CACHE: Dict[Tuple[str, int, int], pd.DataFrame] = {}


def _cached_dataframe_loader(loader: Callable[[str], pd.DataFrame]) -> Callable[[str], pd.DataFrame]:
    @wraps(loader)
    def wrapper(path: str) -> pd.DataFrame:
        ...
        df = loader(str(resolved))
        with _DATAFRAME_CACHE_LOCK:
            stale_keys = [key for key in _DATAFRAME_CACHE if key[0] == cache_key[0] and key != cache_key]
            for stale_key in stale_keys:
                _DATAFRAME_CACHE.pop(stale_key, None)
            _DATAFRAME_CACHE[cache_key] = df.copy(deep=True)
...
if __name__ == "__main__":
    raise SystemExit("backtest_runtime_core.py is backend-only. Run backtest_panel2.py for the Streamlit UI.")
